//cb2024e2c0396665416747683cecad42

/* 1.0 */

// TDI_3971_4573_0146_8888_2525_8264_3791
//-----------------------------------------------------------------
// Fichero  : GetFiles_req.java
 ...
 * @version 1.0
 */
public class GetFiles_req extends java.util.Observable implements es.tid.serial.TdiBean {
     ...
     */
    public GetFiles_req(GetFiles_req object) {
        this.set(object);
    }
     ...
     * @param    object    Objeto a copiar
     */
    public void set(GetFiles_req object) {
        setFilter(object.getFilter());
     ...
     * @param    path    
     */
    public GetFiles_req(org.qualipso.advdoc.ws.client.storage.beans.FolderPath path,String filter) {
        setFilter(filter);
        setPath(path);
    }
    ...
    public String toString() {
        return this.toXML("GetFiles_req",0);
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof GetFiles_req)) {
            return false;
        }
        GetFiles_req value = (GetFiles_req)obj;
        if (!(value.filter.equals(this.filter))) {
            return false;
        }
    ...
    public int hashCode() {
        ...
        int code = 7;
        // Concatenar con el hashcode del atributo filter
        code = tdi.gen.TdiHashcode.concat(code,this.filter);
     ...
     */
    public static boolean equals(GetFiles_req[] __array1,GetFiles_req[] __array2) {
    ...
    /**
     * Lectura del atributo filter
     * @return   Valor del atributo filter
     */
    public String getFilter() { return this.filter; }

    /**
     * Escritura del atributo filter
     * @param    valor    Nuevo valor del atributo filter
     */
    public void setFilter(String valor) { 
        if (valor == null) {
            valor = new String();
        }
        this.filter = valor;
  ...
  public void deserialize(TdiInputSerializer input) throws Exception {
    this.filter = input.extract_String();
  ...
  public String toJS(boolean root,boolean create,String varName,int indents) {
    ...
    if (create) {
        ...
        js.append(varName+".__constructor__ = 'GetFiles_req';\n");
        js.append(varName+".__package__ = 'org.qualipso.advdoc.ws.client.storage.beans';\n");
    }
    js.append(varName+".filter = '"+es.tid.serial.TdiSerialUtil.escapaJavascript(this.filter)+"';\n");
  ...
  public String toXML(String tag, int indents,boolean ponerIndicesDeArrays,boolean escaparStrings) {
    ...
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"<"+tag+" type=\"GetFiles_req\" package=\"org.qualipso.advdoc.ws.client.storage.beans\">");
      xml.append("\n");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents));
      xml.append("<?xml version='1.0' encoding='ISO-8859-1'?>\n");
      xml.append(_indentar(indents)+"<GetFiles_req type=\"GetFiles_req\" package=\"org.qualipso.advdoc.ws.client.storage.beans\">");
      xml.append("\n");
    } else {
      indents--;
    }
    if (escaparStrings) {
        xml.append(_indentar(indents+1)+"<filter type=\"string\">"+tdi.text.StringUtil.escapeXML(this.filter,false)+"</filter>");
    } else {
        xml.append(_indentar(indents+1)+"<filter type=\"string\">"+this.filter+"</filter>");
    }
    ...
    if (tag != null && tag.length() > 0) {
      ...
      xml.append(_indentar(indents)+"</GetFiles_req>");
    }
    return xml.toString();
  }
  ...
  protected Object toRPC(java.util.Hashtable __struct__) {
    __struct__.put("filter", tdi.text.StringUtil.sustituyeUnicode(this.filter));
  ...
  public void fromRPC(java.util.Hashtable __params__) throws Exception {
    Object __object_filter = __params__.get("filter");
    if (__object_filter == null) {
        throw new Exception(getClass().getName()+".fromRPC() No se encuentra el atributo filter");
    }
    if (!(__object_filter instanceof String)) {
        throw new Exception(getClass().getName()+".fromRPC() El atributo filter no es de tipo String");
    }
    setFilter((String)__object_filter);
  ...
  public void fromXML(org.w3c.dom.Element __node) throws Exception {
    {
      String __v = "";
      org.w3c.dom.Element __e = getNodeByName(__node,"filter");
    ...
    public void toSOAP(java.io.Writer writer) throws java.io.IOException {
        writer.write("<filter>");
        writer.write(tdi.text.StringUtil.escapeXML(this.getFilter(), false));
        writer.write("</filter>");
        writer.write("\n");
        writer.write("<path>");
    ...
    public void fromSOAP(tdi.xml.parser.TdiXmlNode beanNode) {
        ...
        node = beanNode.getFirstChildByName("filter");
        if (node != null && node.getContents() != null) {
            this.setFilter(node.getContents());
        }
    ...
    public void toJSON(JSONObject json) throws JSONException {
        JSONObject jObject;
        json.put("filter",this.getFilter());
    ...
    public void fromJSON(JSONObject json) throws JSONException {
        this.setFilter(json.getString("filter"));
