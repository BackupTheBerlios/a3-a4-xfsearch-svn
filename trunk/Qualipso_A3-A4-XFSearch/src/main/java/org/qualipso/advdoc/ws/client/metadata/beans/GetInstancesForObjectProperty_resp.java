//ef26323c0c599219f1771dc2348597d

/* 1.0 */

// TDI_3971_4573_0146_8888_2525_8264_3791
//-----------------------------------------------------------------
// Fichero  : GetInstancesForObjectProperty_resp.java
 ...
 * @version 1.0
 */
public class GetInstancesForObjectProperty_resp extends java.util.Observable implements es.tid.serial.TdiBean {
     ...
     * @composed string
     */
    protected ObjectVector instanceURIs = new ObjectVector(10);
     ...
     */
    public GetInstancesForObjectProperty_resp() {
        // Registrarse como Observer de los atributos hijos
        this.instanceURIs.addObserver(this);
    }
     ...
     */
    public GetInstancesForObjectProperty_resp(GetInstancesForObjectProperty_resp object) {
        this.set(object);
    }
     ...
     * @param    object    Objeto a copiar
     */
    public void set(GetInstancesForObjectProperty_resp object) {
        setInstanceURIs(object.getInstanceURIs());
        setChanged();
    }
     ...
     * @param    instanceURIs    
     */
    public GetInstancesForObjectProperty_resp(String[] instanceURIs) {
        for (int __i = 0;__i < instanceURIs.length;__i++) {
            addInstanceURIs(instanceURIs[__i]);
        }
    }
    ...
    public String toString() {
        return this.toXML("GetInstancesForObjectProperty_resp",0);
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof GetInstancesForObjectProperty_resp)) {
            return false;
        }
        GetInstancesForObjectProperty_resp value = (GetInstancesForObjectProperty_resp)obj;
        if (value.instanceURIs.size() != this.instanceURIs.size()) {
            return false;
        }
        for (int i = 0;i < value.instanceURIs.size();i++) {
            if (!(value.getInstanceURIs(i).equals(this.getInstanceURIs(i)))) {
                return false;
            }
        }
        return true;
    ...
    public int hashCode() {
        ...
        int code = 7;
        // Concatenar con el hashcode del atributo instanceURIs
        for (int i = 0;i < instanceURIs.size();i++) {
            code = tdi.gen.TdiHashcode.concat(code,this.getInstanceURIs(i));
        }
        return code;
    }
     ...
     */
    public static boolean equals(GetInstancesForObjectProperty_resp[] __array1,GetInstancesForObjectProperty_resp[] __array2) {
    ...
    /**
     * Vacia los elementos del atributo instanceURIs
     */
    public void clearInstanceURIs() { this.instanceURIs.clear(); setChanged(); }

    /**
     * Eliminar el elemento i-ésimo del atributo instanceURIs
     * @param    position    Posición del elemento que se quiere eliminar
     * @return   false si no se puede eliminar
     */
public     boolean removeInstanceURIs(int position) {
        return this.instanceURIs.remove(position);
    }

    /**
     * Número de elementos en el atributo instanceURIs
     * @return   Número de elementos
     */
    public int instanceURIs_size() { return this.instanceURIs.size(); }

    /**
     * Elementos del atributo instanceURIs
     * @return   Array de elementos
     */
    public String[] getInstanceURIs() {
        return (String[])this.instanceURIs.toArray(new String[this.instanceURIs.size()]);
    }

    /**
     * Obtener el elemento i-isimo del atributo instanceURIs
     * @param    position    Posicisn del elemento que se quiere obtener
     * @return   Elemento i-isimo
     */
    public String getInstanceURIs(int position) {
        return (String)this.instanceURIs.elementAt(position);
    }

    /**
     * Modificar el elemento i-isimo del atributo instanceURIs
     * @param    position    Posicisn del elemento que se quiere modificar
     * @param    valor    Nuevo elemento
     */
    public void setInstanceURIs(int position,String valor) {
        if (valor == null) {
            valor = new String();
        }
        this.instanceURIs.setElementAt(valor, position);
        setChanged();
    }

    /**
     * Modifica todos los elementos del atributo instanceURIs
     * @param    valores    Los nuevos elementos
     */
    public void setInstanceURIs(String[] valores) {
        this.instanceURIs = new ObjectVector(valores);
        this.instanceURIs.addObserver(this);
        setChanged();
    }

    /**
     * Aqade un nuevo elemento al atributo instanceURIs
     * @param    valor    Nuevo elemento
     */
    public void addInstanceURIs(String valor) {
        if (valor == null) {
            valor = new String();
        }
        this.instanceURIs.addElement(valor);
        setChanged();
    }
    ...
    public static String getStructure() {
        if (estructura == null) {
            String[] lines = new String[] {
                "GetInstancesForObjectProperty_resp <BR>",
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;string[] instanceURIs<BR>",
            };
    ...
    public void fillSampleData() {
        this.instanceURIs.clear();
        this.addInstanceURIs("string_0");
        this.addInstanceURIs("string_1");
    }
  ...
  public void serialize(TdiOutputSerializer output) {
    output.append((int)this.instanceURIs.size());
    for (int i=0; i<this.instanceURIs.size(); i++) {
      output.append((String)this.instanceURIs.elementAt(i));
    }
  }
  ...
  public void deserialize(TdiInputSerializer input) throws Exception {
    {
        ...
        this.instanceURIs = new ObjectVector(len);
        this.instanceURIs.addObserver(this);
        for (int i = 0; i < len; i++) {
          String aux = input.extract_String();
          this.instanceURIs.addElement(aux);
        }
    }
    setChanged();
  }
  ...
  public String toJS(boolean root,boolean create,String varName,int indents) {
    ...
    if (create) {
        ...
        js.append(varName+".__constructor__ = 'GetInstancesForObjectProperty_resp';\n");
        js.append(varName+".__package__ = 'org.qualipso.advdoc.ws.client.metadata.beans';\n");
    }
    js.append(varName+".instanceURIs = new Array("+this.instanceURIs.size()+");\n");
    for (int i = 0;i < this.instanceURIs.size();i++) {
        js.append(varName+".instanceURIs["+i+"] = '"+es.tid.serial.TdiSerialUtil.escapaJavascript((String)this.instanceURIs.elementAt(i))+"';\n");
    }
    return js.toString();
  }
  ...
  public String toXML(String tag, int indents,boolean ponerIndicesDeArrays,boolean escaparStrings) {
    ...
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"<"+tag+" type=\"GetInstancesForObjectProperty_resp\" package=\"org.qualipso.advdoc.ws.client.metadata.beans\">");
      xml.append("\n");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents));
      xml.append("<?xml version='1.0' encoding='ISO-8859-1'?>\n");
      xml.append(_indentar(indents)+"<GetInstancesForObjectProperty_resp type=\"GetInstancesForObjectProperty_resp\" package=\"org.qualipso.advdoc.ws.client.metadata.beans\">");
      xml.append("\n");
    } else {
      indents--;
    }
    xml.append(_indentar(indents+1)+"<instanceURIs type=\"string[]\" size=\""+this.instanceURIs.size()+"\">");
    xml.append("\n");
    for (int i=0; i<this.instanceURIs.size(); i++) {
        String sufijo = (ponerIndicesDeArrays ? ""+i : "elemento");
      xml.append(_indentar(indents+2)+"<instanceURIs"+"_"+sufijo+" type=\"string\">"+instanceURIs.elementAt(i)+"</instanceURIs"+"_"+sufijo+">");
      xml.append("\n");
    }
    xml.append(_indentar(indents+1));
    xml.append("</instanceURIs>");
    xml.append("\n");
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"</"+tag+">");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents)+"</GetInstancesForObjectProperty_resp>");
    }
    return xml.toString();
  }
  ...
  protected Object toRPC(java.util.Hashtable __struct__) {
    java.util.Vector __vector_instanceURIs = new java.util.Vector();
    for (int __i__ = 0;__i__ < instanceURIs.size();__i__++) {
        __vector_instanceURIs.addElement(tdi.text.StringUtil.sustituyeUnicode((String)this.instanceURIs.elementAt(__i__)));
    }
    __struct__.put("instanceURIs",__vector_instanceURIs);
    return __struct__;
  }
  ...
  public void fromRPC(java.util.Hashtable __params__) throws Exception {
    Object __object_instanceURIs = __params__.get("instanceURIs");
    if (__object_instanceURIs == null) {
        throw new Exception(getClass().getName()+".fromRPC() No se encuentra el atributo vector instanceURIs");
    }
    if (!(__object_instanceURIs instanceof java.util.Vector)) {
        throw new Exception(getClass().getName()+".fromRPC() El atributo instanceURIs no es de tipo Vector");
    }
    java.util.Vector __vector_instanceURIs = (java.util.Vector)__object_instanceURIs;
    for (int i = 0;i < __vector_instanceURIs.size();i++) {
        Object __element__ = __vector_instanceURIs.elementAt(i);
        if (!(__element__ instanceof String)) {
            throw new Exception(getClass().getName()+".fromRPC() El elemento "+i+" del array instanceURIs no es de tipo String");
        }
        addInstanceURIs((String)__element__);
    }
    setChanged();
  }
  ...
  public void fromXML(org.w3c.dom.Element __node) throws Exception {
    {
      ...
      org.w3c.dom.Element __element = getNodeByName(__node,"instanceURIs");
      int __size = 0;
      if (__element != null) {
      ...
      for (int i = 0;i < __size;i++) {
        String __v = getNodeByName(__element,"instanceURIs_"+i).getChildNodes().item(0).getNodeValue();
        addInstanceURIs(__v);
      }
    }
    ...
    public void toSOAP(java.io.Writer writer) throws java.io.IOException {
        writer.write("<instanceURIs>");
        writer.write("\n");
        for (int i = 0; i < instanceURIs_size(); i++) {
            writer.write("<string>");
        writer.write(this.getInstanceURIs(i));
            writer.write("</string>");
            writer.write("\n");
        }
        writer.write("</instanceURIs>");
    ...
    public void toJSON(JSONObject json) throws JSONException {
        ...
        for (int i = 0;i < instanceURIs_size();i++) {
            jArray.put(getInstanceURIs(i));
        }
        json.put("instanceURIs",jArray);
    }
    ...
    public void fromJSON(JSONObject json) throws JSONException {
        JSONArray jArray;
        JSONArray jObjectArray;
        jArray = json.getJSONArray("instanceURIs");
        this.clearInstanceURIs();
        for (int i = 0;i < jArray.length();i++) {
            this.addInstanceURIs(jArray.getString(i));
        }
    }
  ...
  private org.w3c.dom.Element getNodeByName(org.w3c.dom.Element parent,String name) throws Exception {
     ...
     */
    public Object clone() {
        return new GetInstancesForObjectProperty_resp(this);
    }
    /**
    ...
    public void copyFrom(Object source) throws ClassCastException {
        if (!(source instanceof GetInstancesForObjectProperty_resp)) {
            throw new ClassCastException(getClass().getName()+".copyFrom() El objeto de copia es de la clase "+source.getClass().getName());
        }
        GetInstancesForObjectProperty_resp bean = (GetInstancesForObjectProperty_resp)source;
        this.set(bean);
    }
