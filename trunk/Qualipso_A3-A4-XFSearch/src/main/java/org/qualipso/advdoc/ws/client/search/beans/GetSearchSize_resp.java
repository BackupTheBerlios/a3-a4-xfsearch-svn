//b96d82e05b357c4c1f8fca27a620501a

/* 1.0 */

// TDI_3971_4573_0146_8888_2525_8264_3791
//-----------------------------------------------------------------
// Fichero  : GetSearchSize_resp.java
 ...
 * @version 1.0
 */
public class GetSearchSize_resp extends java.util.Observable implements es.tid.serial.TdiBean {
     ...
     */
    public GetSearchSize_resp(GetSearchSize_resp object) {
        this.set(object);
    }
     ...
     * @param    object    Objeto a copiar
     */
    public void set(GetSearchSize_resp object) {
        setSearchText(object.getSearchText());
     ...
     * @param    size    
     */
    public GetSearchSize_resp(int size,String searchText) {
    ...
    public String toString() {
        return this.toXML("GetSearchSize_resp",0);
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof GetSearchSize_resp)) {
            return false;
        }
        GetSearchSize_resp value = (GetSearchSize_resp)obj;
        if (!(value.searchText.equals(this.searchText))) {
            return false;
        }
    ...
    public int hashCode() {
        ...
        int code = 7;
        // Concatenar con el hashcode del atributo searchText
        code = tdi.gen.TdiHashcode.concat(code,this.searchText);
        // Concatenar con el hashcode del atributo size
        code = tdi.gen.TdiHashcode.concat(code,this.size);
        return code;
    }
     ...
     */
    public static boolean equals(GetSearchSize_resp[] __array1,GetSearchSize_resp[] __array2) {
    ...
    /**
     * Lectura del atributo searchText
     * @return   Valor del atributo searchText
     */
    public String getSearchText() { return this.searchText; }

    /**
     * Escritura del atributo searchText
     * @param    valor    Nuevo valor del atributo searchText
     */
    public void setSearchText(String valor) { 
        if (valor == null) {
            valor = new String();
        }
    ...
    /**
     * Lectura del atributo size
     * @return   Valor del atributo size
     */
    public int getSize() { return this.size; }

    /**
     * Escritura del atributo size
     * @param    valor    Nuevo valor del atributo size
     */
  ...
  public void deserialize(TdiInputSerializer input) throws Exception {
    this.searchText = input.extract_String();
    this.size = input.extract_int();
    setChanged();
  }
  ...
  public String toJS(boolean root,boolean create,String varName,int indents) {
    ...
    if (create) {
        ...
        js.append(varName+".__constructor__ = 'GetSearchSize_resp';\n");
        js.append(varName+".__package__ = 'org.qualipso.advdoc.ws.client.search.beans';\n");
    }
    js.append(varName+".searchText = '"+es.tid.serial.TdiSerialUtil.escapaJavascript(this.searchText)+"';\n");
    js.append(varName+".size = "+this.size+";\n");
    return js.toString();
  }
  ...
  public String toXML(String tag, int indents,boolean ponerIndicesDeArrays,boolean escaparStrings) {
    ...
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"<"+tag+" type=\"GetSearchSize_resp\" package=\"org.qualipso.advdoc.ws.client.search.beans\">");
      xml.append("\n");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents));
      xml.append("<?xml version='1.0' encoding='ISO-8859-1'?>\n");
      xml.append(_indentar(indents)+"<GetSearchSize_resp type=\"GetSearchSize_resp\" package=\"org.qualipso.advdoc.ws.client.search.beans\">");
      xml.append("\n");
    } else {
      indents--;
    }
    if (escaparStrings) {
        xml.append(_indentar(indents+1)+"<searchText type=\"string\">"+tdi.text.StringUtil.escapeXML(this.searchText,false)+"</searchText>");
    } else {
        xml.append(_indentar(indents+1)+"<searchText type=\"string\">"+this.searchText+"</searchText>");
    }

    xml.append("\n");
    xml.append(_indentar(indents+1)+"<size type=\"int\">"+this.size+"</size>");
    xml.append("\n");
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"</"+tag+">");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents)+"</GetSearchSize_resp>");
    }
    return xml.toString();
  }
  ...
  protected Object toRPC(java.util.Hashtable __struct__) {
    __struct__.put("searchText", tdi.text.StringUtil.sustituyeUnicode(this.searchText));
    __struct__.put("size", new Integer(size));
    return __struct__;
  }
  ...
  public void fromRPC(java.util.Hashtable __params__) throws Exception {
    Object __object_searchText = __params__.get("searchText");
    if (__object_searchText == null) {
        throw new Exception(getClass().getName()+".fromRPC() No se encuentra el atributo searchText");
    }
    if (!(__object_searchText instanceof String)) {
        throw new Exception(getClass().getName()+".fromRPC() El atributo searchText no es de tipo String");
    }
    setSearchText((String)__object_searchText);
    Object __object_size = __params__.get("size");
    if (__object_size == null) {
        throw new Exception(getClass().getName()+".fromRPC() No se encuentra el atributo size");
    }
    if (!(__object_size instanceof Integer)) {
        throw new Exception(getClass().getName()+".fromRPC() El atributo size no es de tipo Integer");
    }
    Integer __int_size = (Integer)__object_size;
    setSize(__int_size.intValue());
    setChanged();
  }
  ...
  public void fromXML(org.w3c.dom.Element __node) throws Exception {
    {
      String __v = "";
      org.w3c.dom.Element __e = getNodeByName(__node,"searchText");
    ...
    {
      String __v = "";
      org.w3c.dom.Element __e = getNodeByName(__node,"size");
      if (__e != null && __e.getChildNodes().item(0) != null) {
        __v = __e.getChildNodes().item(0).getNodeValue();
      }
      setSize(Integer.parseInt(__v));
    }
    ...
    public void toSOAP(java.io.Writer writer) throws java.io.IOException {
        ...
        writer.write(tdi.text.StringUtil.escapeXML(this.getSearchText(), false));
        writer.write("</searchText>");
        writer.write("\n");
        writer.write("<size>");
        writer.write(Integer.toString(this.getSize()));
        writer.write("</size>");
        writer.write("\n");
    }
    ...
    public void fromSOAP(tdi.xml.parser.TdiXmlNode beanNode) {
        ...
        node = beanNode.getFirstChildByName("searchText");
        if (node != null && node.getContents() != null) {
            this.setSearchText(node.getContents());
        }

        node = beanNode.getFirstChildByName("size");
        if (node != null && node.getContents() != null) {
            this.setSize(Integer.parseInt(node.getContents()));
        }
    ...
    public void toJSON(JSONObject json) throws JSONException {
        JSONObject jObject;
        json.put("searchText",this.getSearchText());
    ...
    public void fromJSON(JSONObject json) throws JSONException {
        this.setSearchText(json.getString("searchText"));
        this.setSize(json.getInt("size"));
        JSONArray jArray;
        JSONArray jObjectArray;
    }
  ...
  private org.w3c.dom.Element getNodeByName(org.w3c.dom.Element parent,String name) throws Exception {
     ...
     */
    public Object clone() {
        return new GetSearchSize_resp(this);
    }
    /**
