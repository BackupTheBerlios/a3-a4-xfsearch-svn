//413ca7eb6a527a11695136f432a

/* 1.0 */

// TDI_3971_4573_0146_8888_2525_8264_3791
//-----------------------------------------------------------------
// Fichero  : OptionalConditionGroup.java
 ...
 * @version 1.0
 */
public class OptionalConditionGroup extends java.util.Observable implements es.tid.serial.TdiBean {
     ...
     * @composed string
     */
    protected ObjectVector optionalCondition = new ObjectVector(10);
     ...
     */
    public OptionalConditionGroup() {
        // Registrarse como Observer de los atributos hijos
        this.optionalCondition.addObserver(this);
    }
     ...
     */
    public OptionalConditionGroup(OptionalConditionGroup object) {
        this.set(object);
    }
     ...
     * @param    object    Objeto a copiar
     */
    public void set(OptionalConditionGroup object) {
        setOptionalCondition(object.getOptionalCondition());
        setChanged();
    }
     ...
     */
    public OptionalConditionGroup(String[] optionalCondition) {
        for (int __i = 0;__i < optionalCondition.length;__i++) {
            addOptionalCondition(optionalCondition[__i]);
        }
    }
    ...
    public String toString() {
        return this.toXML("OptionalConditionGroup",0);
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof OptionalConditionGroup)) {
            return false;
        }
        OptionalConditionGroup value = (OptionalConditionGroup)obj;
        if (value.optionalCondition.size() != this.optionalCondition.size()) {
            return false;
        }
        for (int i = 0;i < value.optionalCondition.size();i++) {
            if (!(value.getOptionalCondition(i).equals(this.getOptionalCondition(i)))) {
                return false;
            }
        }
        return true;
    ...
    public int hashCode() {
        ...
        int code = 7;
        // Concatenar con el hashcode del atributo optionalCondition
        for (int i = 0;i < optionalCondition.size();i++) {
            code = tdi.gen.TdiHashcode.concat(code,this.getOptionalCondition(i));
        }
        return code;
    }
     ...
     */
    public static boolean equals(OptionalConditionGroup[] __array1,OptionalConditionGroup[] __array2) {
    ...
    /**
     * Vacia los elementos del atributo optionalCondition
     */
    public void clearOptionalCondition() { this.optionalCondition.clear(); setChanged(); }

    /**
     * Eliminar el elemento i-ésimo del atributo optionalCondition
     * @param    position    Posición del elemento que se quiere eliminar
     * @return   false si no se puede eliminar
     */
public     boolean removeOptionalCondition(int position) {
        return this.optionalCondition.remove(position);
    }

    /**
     * Número de elementos en el atributo optionalCondition
     * @return   Número de elementos
     */
    public int optionalCondition_size() { return this.optionalCondition.size(); }

    /**
     * Elementos del atributo optionalCondition
     * @return   Array de elementos
     */
    public String[] getOptionalCondition() {
        return (String[])this.optionalCondition.toArray(new String[this.optionalCondition.size()]);
    }

    /**
     * Obtener el elemento i-isimo del atributo optionalCondition
     * @param    position    Posicisn del elemento que se quiere obtener
     * @return   Elemento i-isimo
     */
    public String getOptionalCondition(int position) {
        return (String)this.optionalCondition.elementAt(position);
    }

    /**
     * Modificar el elemento i-isimo del atributo optionalCondition
     * @param    position    Posicisn del elemento que se quiere modificar
     * @param    valor    Nuevo elemento
     */
    public void setOptionalCondition(int position,String valor) {
        if (valor == null) {
            valor = new String();
        }
        this.optionalCondition.setElementAt(valor, position);
        setChanged();
    }

    /**
     * Modifica todos los elementos del atributo optionalCondition
     * @param    valores    Los nuevos elementos
     */
    public void setOptionalCondition(String[] valores) {
        this.optionalCondition = new ObjectVector(valores);
        this.optionalCondition.addObserver(this);
        setChanged();
    }

    /**
     * Aqade un nuevo elemento al atributo optionalCondition
     * @param    valor    Nuevo elemento
     */
    public void addOptionalCondition(String valor) {
        if (valor == null) {
            valor = new String();
        }
        this.optionalCondition.addElement(valor);
        setChanged();
    }
    ...
    public void fillSampleData() {
        this.optionalCondition.clear();
        this.addOptionalCondition("string_0");
        this.addOptionalCondition("string_1");
    }
  ...
  public void serialize(TdiOutputSerializer output) {
    output.append((int)this.optionalCondition.size());
    for (int i=0; i<this.optionalCondition.size(); i++) {
      output.append((String)this.optionalCondition.elementAt(i));
    }
  }
  ...
  public void deserialize(TdiInputSerializer input) throws Exception {
    {
        ...
        this.optionalCondition = new ObjectVector(len);
        this.optionalCondition.addObserver(this);
        for (int i = 0; i < len; i++) {
          String aux = input.extract_String();
          this.optionalCondition.addElement(aux);
        }
    }
    setChanged();
  }
  ...
  public String toJS(boolean root,boolean create,String varName,int indents) {
    ...
    if (create) {
        ...
        js.append(varName+".__constructor__ = 'OptionalConditionGroup';\n");
        js.append(varName+".__package__ = 'org.qualipso.advdoc.ws.client.search.beans';\n");
    }
    js.append(varName+".optionalCondition = new Array("+this.optionalCondition.size()+");\n");
    for (int i = 0;i < this.optionalCondition.size();i++) {
        js.append(varName+".optionalCondition["+i+"] = '"+es.tid.serial.TdiSerialUtil.escapaJavascript((String)this.optionalCondition.elementAt(i))+"';\n");
    }
    return js.toString();
  }
  ...
  public String toXML(String tag, int indents,boolean ponerIndicesDeArrays,boolean escaparStrings) {
    ...
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"<"+tag+" type=\"OptionalConditionGroup\" package=\"org.qualipso.advdoc.ws.client.search.beans\">");
      xml.append("\n");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents));
      xml.append("<?xml version='1.0' encoding='ISO-8859-1'?>\n");
      xml.append(_indentar(indents)+"<OptionalConditionGroup type=\"OptionalConditionGroup\" package=\"org.qualipso.advdoc.ws.client.search.beans\">");
      xml.append("\n");
    } else {
      indents--;
    }
    xml.append(_indentar(indents+1)+"<optionalCondition type=\"string[]\" size=\""+this.optionalCondition.size()+"\">");
    xml.append("\n");
    for (int i=0; i<this.optionalCondition.size(); i++) {
        String sufijo = (ponerIndicesDeArrays ? ""+i : "elemento");
      xml.append(_indentar(indents+2)+"<optionalCondition"+"_"+sufijo+" type=\"string\">"+optionalCondition.elementAt(i)+"</optionalCondition"+"_"+sufijo+">");
      xml.append("\n");
    }
    xml.append(_indentar(indents+1));
    xml.append("</optionalCondition>");
    xml.append("\n");
    if (tag != null && tag.length() > 0) {
      xml.append(_indentar(indents)+"</"+tag+">");
    } else if (tag != null && tag.length() == 0) {
      xml.append(_indentar(indents)+"</OptionalConditionGroup>");
    }
    return xml.toString();
  }
  ...
  protected Object toRPC(java.util.Hashtable __struct__) {
    java.util.Vector __vector_optionalCondition = new java.util.Vector();
    for (int __i__ = 0;__i__ < optionalCondition.size();__i__++) {
        __vector_optionalCondition.addElement(tdi.text.StringUtil.sustituyeUnicode((String)this.optionalCondition.elementAt(__i__)));
    }
    __struct__.put("optionalCondition",__vector_optionalCondition);
    return __struct__;
  }
  ...
  public void fromRPC(java.util.Hashtable __params__) throws Exception {
    Object __object_optionalCondition = __params__.get("optionalCondition");
    if (__object_optionalCondition == null) {
        throw new Exception(getClass().getName()+".fromRPC() No se encuentra el atributo vector optionalCondition");
    }
    if (!(__object_optionalCondition instanceof java.util.Vector)) {
        throw new Exception(getClass().getName()+".fromRPC() El atributo optionalCondition no es de tipo Vector");
    }
    java.util.Vector __vector_optionalCondition = (java.util.Vector)__object_optionalCondition;
    for (int i = 0;i < __vector_optionalCondition.size();i++) {
        Object __element__ = __vector_optionalCondition.elementAt(i);
        if (!(__element__ instanceof String)) {
            throw new Exception(getClass().getName()+".fromRPC() El elemento "+i+" del array optionalCondition no es de tipo String");
        }
        addOptionalCondition((String)__element__);
    }
    setChanged();
  }
  ...
  public void fromXML(org.w3c.dom.Element __node) throws Exception {
    {
      ...
      org.w3c.dom.Element __element = getNodeByName(__node,"optionalCondition");
      int __size = 0;
      if (__element != null) {
      ...
      for (int i = 0;i < __size;i++) {
        String __v = getNodeByName(__element,"optionalCondition_"+i).getChildNodes().item(0).getNodeValue();
        addOptionalCondition(__v);
      }
    }
    ...
    public void toSOAP(java.io.Writer writer) throws java.io.IOException {
        writer.write("<optionalCondition>");
        writer.write("\n");
        for (int i = 0; i < optionalCondition_size(); i++) {
            writer.write("<string>");
        writer.write(this.getOptionalCondition(i));
            writer.write("</string>");
            writer.write("\n");
        }
        writer.write("</optionalCondition>");
    ...
    public void fromSOAP(tdi.xml.parser.TdiXmlNode beanNode) {
        ...
        node = beanNode.getFirstChildByName("optionalCondition");
        if (node != null && node.getContents() != null) {

            arrayNodes = node.getChildsByName("string");
            if (arrayNodes != null) {
                this.clearOptionalCondition();
                for (int i = 0;i < arrayNodes.length;i++) {
                    if (arrayNodes[i] != null && arrayNodes[i].getContents() != null) {
                        this.addOptionalCondition(arrayNodes[i].getContents());
                    }  // arrayNodes[i] != null
                }  // for
            }  // arrayNodes != null
        }  // node != null
    ...
    public void toJSON(JSONObject json) throws JSONException {
        ...
        for (int i = 0;i < optionalCondition_size();i++) {
            jArray.put(getOptionalCondition(i));
        }
        json.put("optionalCondition",jArray);
    }
    ...
    public void fromJSON(JSONObject json) throws JSONException {
        JSONArray jArray;
        JSONArray jObjectArray;
        jArray = json.getJSONArray("optionalCondition");
        this.clearOptionalCondition();
        for (int i = 0;i < jArray.length();i++) {
            this.addOptionalCondition(jArray.getString(i));
        }
    }
  ...
  private org.w3c.dom.Element getNodeByName(org.w3c.dom.Element parent,String name) throws Exception {
     ...
     */
    public Object clone() {
        return new OptionalConditionGroup(this);
    }
    /**
